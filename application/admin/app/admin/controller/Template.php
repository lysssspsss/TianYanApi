...
class Template extends Common{
    ...
    public function debris(){
		...
		$debris=db('debris')->where($map)->select();
		$this->assign('debris',$debris);
		$this->assign('val',$val);
		return $this->fetch();
	}
	public function addDebris(){
		return $this->fetch();
	}
	public function debrisInsert(){
		$data = input('post.');
		$data['addtime'] = time();
		db('debris')->insert($data);
		$result['status'] = 1;
		$result['info'] = '碎片添加成功!';
		$result['url'] = url('debris');
		return $result;
	}
	public function debrisDel(){
		db('debris')->where('id',input('param.id'))->delete();
		$this->redirect('debris');
	}
	public function debrisEdit(){
		$info = db('debris')->where('id',input('param.id'))->find();
		$this->assign('info',$info);
		return $this->fetch();
	}
	public function debrisUpdate(){
		$data = input('post.');
		$data['addtime'] = time();
