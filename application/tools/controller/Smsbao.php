...
class Smsbao {
   ...
   /* private $account = DXB_ACCOUNT;//短信宝账户
    private $password = DXB_PASSWORD;//密码*/
    private $balance;//短信剩余条数
    private static $sendSmsUrl = DXB_SENDSMSURL;
    private static $queryBalanceUrl = DXB_QUERYURL;
    /**
     * 架构函数
     * @access public
...
//    }
    ...
    public static function sendSms($mobile,$content){
        $param['u'] = DXB_ACCOUNT;
        $param['p'] = md5(DXB_PASSWORD);
        $param['m'] = $mobile;
        $param['c'] = $content;
        $ret = self::http(self::$sendSmsUrl, $param);
        $data['status'] = $ret;
        $data['msg'] = $ret == 0 ?'success' : self::getResult($ret);
        return $data;
    }
   ...
   /**
    ...
    protected static function http($url, $param, $data = '', $method = 'GET'){
        ...
        //发生错误
        if($error) {
            wlog(APP_PATH.'log/User.log','http 请求发生错误：' . $error);
            return false;
        }

        return  $data;
    }
    private static function getResult($key){
        $rst['30'] = '密码错误';
        $rst['40'] = '账号不存在';
        $rst['41'] = '余额不足';
        $rst['42'] = '帐号过期';
